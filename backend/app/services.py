...
def answer_agent(db: Session, request: AgentRequest) -> AgentResponse:
    ...
    if settings.llm_enabled:
        ...
        evidence = "\n\n".join(evidence_parts)
        recent_history = "\n".join(
            f"{message.role}: {message.content}" for message in request.history[-12:]
        )
        history_context = recent_history if recent_history else "No previous conversation."
        prompt = (
            ...
            "Answer in Korean. Use the recent conversation only when it helps answer the current question. "
            "Put the direct answer first, then a practical career/learning path. "
